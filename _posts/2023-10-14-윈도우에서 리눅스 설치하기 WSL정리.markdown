...
auto attach 명령이 없다면 중간에 끊어졌을때 다시 Host가 권한을 독점을하지만 해당 명령어를 붙여주면 중간에 끊어지더라도 다시 감지되면 wsl로 계속해서 붙을 수 있게 해준다.

참고로 이 기능 자체가 WSL2에서만 가능한것으로 나온다. 
...
```
 ...
 우분투에 직접 해당 명령어를 통해서 Linux에서 USBIP 도구 및 하드웨어 데이터베이스 설치를 하면 된다고 한다.